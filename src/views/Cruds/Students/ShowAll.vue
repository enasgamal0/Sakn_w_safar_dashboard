    ...
    <main>
          ...
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <base-input
                ...
                v-model.trim="filterOptions.student_name"
              />
              <!-- End:: Name Input -->
      ...
      >
        ...
        <template v-slot:[`item.student_name`]="{ item }">
          <h6 class="text-danger" v-if="!item.student_name">
            {{ $t("TABLES.noData") }}
          </h6>
          <h6 v-else>{{ item.student_name }}</h6>
        </template>
        <!-- End:: Name -->
...
export default {
  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.student_name"),
          value: "name",
          align: "center",
          sortable: false,
        },
  ...
  methods: {
    ...
    showItem(item) {
      this.$router.push({ path: `/students/show/${item.id}` });
    },
    // ===== End:: End
    // Start:: Handel Filter
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/students/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
      this.filterOptions.student_name = null;
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            name: this.filterOptions.student_name,
    ...
    async HandlingItemActivationStatus(selectedItem) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `clients/${targetItem.id}/toggle`,
          data: REQUEST_DATA,
        });
        this.$message.success(this.$t("MESSAGES.changeActivation"));
        this.setTableRows();
