    ...
    <main>
      ...
      <div class="table_title_wrapper">
        ...
        <div
          ...
          v-if="$can('study subject create', 'study-subject')"
        >
          <router-link to="/subjects/create">
            {{ $t("PLACEHOLDERS.addSubject") }}
          </router-link>
        </div>
      </div>
      <!--  =========== End:: Table Title =========== -->
      ...
      >
        ...
        <template v-slot:[`item.is_active`]="{ item }">
          ...
          <div
            ...
            dir="ltr"
            style="z-index: 1"
            v-if="$can('study subject activate', 'study-subject')"
          >
            <v-switch
              class="mt-2"
              color="success"
        ...
        <template v-slot:[`item.actiosn`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('study subject delete', 'study-subject')"
            >
              <template slot="title">
            ...
            <a-tooltip
              placement="bottom"
              v-if="$can('study subject edit', 'study-subject')"
            >
              <template slot="title">
            ...
            <a-tooltip placement="bottom" v-if="$can('study subject show', 'study-subject')">
              <template slot="title">
  ...
  methods: {
    ...
    async submitFilterForm() {
      if (this.$route.query.page !== "1") {
        await this.$router.push({ path: "/subjects/all", query: { page: 1 } });
      }
      this.setTableRows();
    },
    async resetFilter() {
    ...
    async changeActivationStatus(item) {
      ...
      try {
        await this.$axios({
          method: "POST",
          url: `study-subject/${item.id}/toggle`,
          data: REQUEST_DATA,
        });
        this.setTableRows();
    ...
    // ===== Start:: End
    editItem(item) {
      this.$router.push({ path: `/subjects/edit/${item.id}` });
    },
    showItem(item) {
      this.$router.push({ path: `/subjects/show/${item.id}` });
    },
    // ===== End:: End
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `study-subject/${this.itemToDelete.id}`,
        });
