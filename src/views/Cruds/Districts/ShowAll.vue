  ...
  data() {
    return {
      ...
      tableHeaders: [
        ...
        {
          text: this.$t("PLACEHOLDERS.state"),
  ...
  methods: {
    ...
    async getCities() {
      try {
        let res = await this.$axios({
          method: "GET",
          url: `cities?page=0&limit=0&is_active=1`,
        });
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            state: this.filterOptions.state?.id,
            city: this.filterOptions.city?.id,
